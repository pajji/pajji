...
> Stored XSS에서만 
Form URL, Process URL 구분

> SQL_i : 똑같은 유형으로 중복되어 나타난다.
대표 케이스 1개만 쓴다.
> 공격과정을 다 작성할 필요는 없다  / 접속 > 공격수행 끝.
개발자는 공격과정이 궁금하지 않다

이행점검 때 없던 취약점을 찾았을 때
...
SB커뮤니티 모의해킹 결과보고서 완성본을 만들어봐라 / 포트폴리오
————————————————————————————————————
취약점으로 공격자가 뭘 할 수 있냐? 왜 위험하냐

1 불충분한 세션 종료 처리
> 가만히 시간 오래 있어도 유지되면
> 로그아웃을 했는데, 세션이 남아있는 경우 / Logout.php > session 파기 안함
> 웹 브라우저 닫았을 때 세션이 남아있는거 이거는 취약점이 아니다. 서버에서는 브라우저가 꺼진지 모른다.
> 공격자가 만약 세션을 탈취했다면, 탈취한 세션을 영구적으로 사용할 수 있다. / 때문에 취약점이다

** 세션정보 재사용
> 세션 아이디 탈취한 다음에 공격자가 쓰는 것이 문제

[대응 방법]
중복 로그인 탐지 : 로그인 처리할 때 할 수 있다.

중복세션 탐지
> IP를 활용 : 세션에 IP주소를 저장해 놓고, 접속시 대조
> MAC 주소로 활용 : 웹에서는 안된다. 앱은 가능

2 취약한 HTTPS 4종 세트
https://github.com/hahwul/a2sv
SSL Labs

SSL v3.0 > POOBLE Attack / https://github.com/mpgn/poodle-PoC
> 실제 공격 전제조건 : 사이트에 XSS가 존재해야한다
> 블록 암호
> 패딩을 추가한다
...
SQL_Injeciton
- What : 공격자가 임의의SQL 질의문을 삽입해 서버 측에서 실행되는 SQL 질의문이 공격자의 의도대로 변조되는 공격/
- 
